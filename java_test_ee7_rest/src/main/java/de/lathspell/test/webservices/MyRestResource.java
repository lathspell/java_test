 ...
 * e.g. /java_test_ee7_rest/webresources/ before everything that is defined
...
public class MyRestResource {
    ...
    /**
     * Automatic conversion of POJOs to JSON.
     *
     * @return Effectively a HTTP response with JSON data.
     */
    @GET
    @Path("/person")
    ...
    /**
     * Adjusting the automatic generation of JSON from POJO classes.
     *
     * In this case the attribute "dayOfBirth" is translated to the JSON
    ...
    @GET
    @Path("/person_")
    public Response getPersonWithUnderscores() throws Exception {
        String json = new ObjectMapper().setPropertyNamingStrategy(new LowerCaseWithUnderscoresStrategy()).writeValueAsString(Person.MAX);
